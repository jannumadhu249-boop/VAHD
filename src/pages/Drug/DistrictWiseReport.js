...
//           setDrugIndents(groups)
          
//           // Calculate grand totals
//           let totalBudgetReleased = 0
//           let totalBudgetBooked = 0
//           let totalBalanceBudget = 0
...
//               "Total": "",
//               "GST": "",
//               "Grand Total": `DVAHO: ${group.dvahoName || ""}`,
//             })

//             // Add drug items
//             if (group.drugs && Array.isArray(group.drugs)) {
//               group.drugs.forEach((item, index) => {
//                 csvData.push({
//                   "No.": index + 1,
//                   "Drug Code": item.drugCode || "-",
//                   "Trade Name": item.tradeName || "-",
//                   "Composition and Strength": item.compositionAndStrength || "-",
...
//                   "Total": formatNumber(item.totalAmount, 2),
//                   "GST": item.gstPercent ? `${item.gstPercent}%` : "-",
//                   "Grand Total": formatNumber(item.grandTotal, 2),
//                 })
//               })
//             }
...
//               "Qty": "",
//               "Unit Price": "Total",
//               "Total": formatNumber(group.totalAmount, 2),
//               "GST": formatNumber(group.gstTotal, 2),
//               "Grand Total": formatNumber(group.grandTotal, 2),
//             })

//             // Add budget summary row
//             csvData.push({
//               "No.": "",
//               "Drug Code": "",
//               "Trade Name": "",
...
//               "GST": `Budget Booked: ${formatNumber(group.totalAmount, 2)}`,
//               "Grand Total": `Balance Budget: ${formatNumber(group.gstTotal, 2)}`,
//             })
...
//             // Accumulate totals
//             totalBudgetBooked += parseFloat(group.totalAmount || 0)
//             totalBalanceBudget += parseFloat(group.gstTotal || 0)
//           })
...
//               // "GST": `Total Budget Booked: ${formatNumber(totalBudgetBooked, 2)}`,
//               // "Grand Total": `Total Balance Budget: ${formatNumber(totalBalanceBudget, 2)}`,
//             })
//           }
...
//                             <td colSpan="12">
//                               <h5 className="mb-1">{group.dvahoName}</h5>
//                             </td>
//                           </tr>
//                         </tbody>

//                         <tbody>
//                           {group.drugs.map((item, index) => {
//                             return (
...
//                               {" "}
//                               GRAND TOTAL OF {group.dvahoName}:
//                             </span>

//                             <span className="fw-bold">{group.grandTotal}</span>
//                           </div>
//                         </div>
//                         <div
//                           className="col-md-3"
...
//                             <span className="fw-bold">{group.totalAmount}</span>
//                           </div>
//                         </div>
//                         <div
//                           className="col-md-3"
...
//                             <span className="fw-bold">{group.gstTotal}</span>
//                           </div>
//                         </div>
//                       </div>
//                     </div>
...
const GroupsReport = () => {
    ...
    async (customFilters = null, customFinancialYearId = null) => {
      ...
      try {
        ...
        if (response.data) {
          ...
          groups.forEach(group => {
            if (group.drugs && Array.isArray(group.drugs)) {
              totalRecords += group.drugs.length
              group.drugs.forEach(item => {
                totalQuantity += parseFloat(item.qty || item.quantity || 0)
                totalAmount += parseFloat(item.grandTotal || item.totalAmount || 0)
              })
            }
          })
  ...
  const generatePDF = () => {
    ...
    try {
      ...
      drugIndents.forEach((group, groupIndex) => {
        ...
        const groupTitle = `${group.dvahoName || ""}`
        doc.text(groupTitle, pageWidth / 2, startY + 5, { align: "center" })
        ...
        if (group.drugs && Array.isArray(group.drugs)) {
          group.drugs.forEach((item, index) => {
            tableData.push([
              ...
              {
                content: formatNumber(item.unitPrice, 2),
                styles: { halign: "right" },
              },
              {
                content: formatNumber(item.totalAmount, 2),
                styles: { halign: "right" },
              },
              {
                content: `${item.gstPercent || 12}%`,
                styles: { halign: "center" },
              },
              {
                content: formatNumber(item.grandTotal, 2),
                styles: { halign: "right" },
              },
            ])
          })
        ...
        autoTable(doc, {
          ...
          head: [
            [
              ...
              { content: "Drug Code", styles: { halign: "center" } },
              { content: "Trade Name", styles: { halign: "center" } },
              { content: "Composition & Strength", styles: { halign: "center" } },
              { content: "Specifications", styles: { halign: "center" } },
              { content: "Unit Pack", styles: { halign: "center" } },
              { content: "Name of Stockiest", styles: { halign: "center" } },
              { content: "Qty", styles: { halign: "center" } },
              { content: "Unit Price", styles: { halign: "center" } },
              { content: "Total", styles: { halign: "center" } },
              { content: "GST", styles: { halign: "center" } },
        ...
        const totalAmountText = `Total Amount: ${formatNumber(group.totalAmount || 0)}`
        doc.text(totalAmountText, leftMargin, startY)

        const gstTotalText = `GST Total: ${formatNumber(group.gstTotal || 0)}`
        doc.text(gstTotalText, leftMargin + 100, startY)

        const grandTotalText = `Grand Total: ${formatNumber(group.grandTotal || 0)}`
        doc.text(grandTotalText, leftMargin + 200, startY)
  ...
  const filteredDrugIndents = useMemo(() => {
    ...
    return drugIndents
      .map(group => {
        if (!group.drugs) return group

        const filteredDrugs = group.drugs.filter(item => {
      ...
      .filter(group => group.drugs && group.drugs.length > 0)
                      ...
                      <React.Fragment key={group._id || groupIndex}>
                        <div className="mb-3 p-1">
                          <Table hover className="table table-bordered mb-4">
                            ...
                            <tbody>
                              {group.drugs && Array.isArray(group.drugs) && group.drugs.map((item, index) => (
                                <tr key={item._id || index} className="text-center">
                                  ...
                                  <td>{item.nameOfStockiest || "-"}</td>
                                  <td className="text-center">{item.qty || 0}</td>
                                  <td className="text-end">{item.unitPrice || 0}</td>
                                  <td className="text-center">
                                    {formatNumber(item?.totalAmount, 2) || "-"}
                                  </td>
                                  <td className="text-end">
