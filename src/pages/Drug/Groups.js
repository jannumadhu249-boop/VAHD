...
import React, { useState, useEffect, useCallback, useMemo } from "react"
import DrugTemplate from "../../assets/images/DrugIndentSample.xlsx"
import { ToastContainer, toast } from "react-toastify"
import "react-toastify/dist/ReactToastify.css"
import ReactPaginate from "react-paginate"
import { CSVLink } from "react-csv"
...
const DrugIndent = () => {
  ...
  const [mandal, setMandal] = useState({ name: "" })
  const [editDistrict, setEditDistrict] = useState({ name: "" })
  const [editMandal, setEditMandal] = useState({ name: "" })
  const [budgetData, setBudgetData] = useState({
    budget: 0,
    bookedBudget: 0,
    availableBudget: 0,
  ...
  const fetchDistrictAndMandalData = useCallback(
    async (workingPlaceId, isEdit = false) => {
      if (!token || !workingPlaceId) {
        if (isEdit) {
          setEditDistrict({ name: "" })
          setEditMandal({ name: "" })
        } else {
          setDistrict({ name: "" })
          setMandal({ name: "" })
      ...
      try {
        ...
        if (data) {
          if (isEdit) {
            setEditDistrict(data.district || { name: "" })
            setEditMandal(data.mandal || { name: "" })
            setEditFormData(prev => ({
              ...prev,
              districtId: data.district?._id || "",
        ...
        console.error("Failed to load district and mandal data:", error)
        toast.error("Failed to load location details")
      }
    },
    [token]
  ...
  const fetchDrugIndents = useCallback(async (filterParams = filters) => {
    if (!token || !formId) {
      toast.error("Authentication token or Form ID not found")
      return
    }
  ...
  const handleBulkUploadInputChange = (e, index = null) => {
    const { name, value } = e.target
    ...
    // Clear district and mandal when working place is cleared
    if (name === "workingPlaceId" && !value) {
      setBulkUploadData(prev => ({
        ...prev,
        districtName: "",
        mandalName: "",
    ...
    // Fetch district when working place is selected (if using input instead of select)
    if (name === "workingPlaceId" && value) {
      fetchDistrictDataForBulkUpload(value)
    }

    // Fetch schemes and quarters when financial year is selected
    if (name === "financialYearId" && value) {
      fetchSchemesAndQuartersForBulkUpload(value)
    }
  }

  const fetchDistrictDataForBulkUpload = async workingPlaceId => {
    if (!token || !workingPlaceId) return
    ...
    try {
      ...
      console.error("Failed to load district data for bulk upload:", error)
    }
  }

  const fetchSchemesAndQuartersForBulkUpload = async financialYearId => {
    if (!token || !financialYearId) return
    ...
    try {
      ...
      console.error("Error fetching Schemes and Quarters for bulk upload:", error)
    }
  }

  const handleBulkUploadSelectChange = (selectedOption, { name }) => {
    const value = selectedOption?.value || "";
    ...
    if (name === "workingPlaceId" && value) {
      // Find the selected place to get district and mandal names
      const selectedPlace = placeOfWorking.find(place => place._id === value);
      setBulkUploadData(prev => ({
        ...prev,
        [name]: value,
        districtName: selectedPlace?.districtName || "",
        mandalName: selectedPlace?.mandalName || "",
      }));
    } else {
      setBulkUploadData(prev => ({
        ...prev,
    ...
    if (name === "institutionTypeId" && value) {
      fetchPlaceOfWorkingForBulkUpload(value);
    }
  };

  const fetchPlaceOfWorkingForBulkUpload = async institutionTypeId => {
    if (!token || !institutionTypeId) {
      setPlaceOfWorking([]);
      return;
    }
    try {
      ...
      console.error("Error fetching places for bulk upload:", error);
      toast.error("Failed to load places of working");
    }
  };

  const getBulkUploadCurrentPlace = () => {
    if (!bulkUploadData.workingPlaceId) return null;
    const place = placeOfWorking.find(p => p._id === bulkUploadData.workingPlaceId);
    return place ? { value: place._id, label: place.name } : null;
  };
  ...
  const validateBulkUpload = () => {
    ...
    if (!bulkUploadData.financialYearId) {
      errors.financialYearId = "Financial Year is required";
    }

    if (!bulkUploadData.schemeId) {
      errors.schemeId = "Scheme is required";
    }

    if (!bulkUploadData.quarterId) {
      errors.quarterId = "Quarter is required";
    }

    if (!bulkUploadData.institutionTypeId) {
      errors.institutionTypeId = "Type of Institution is required";
    }

    if (!bulkUploadData.workingPlaceId) {
      errors.workingPlaceId = "Place of Working is required";
    }
  ...
  const handleBulkUploadSubmit = async (e) => {
    ...
    const errors = validateBulkUpload();
    if (Object.keys(errors).length > 0) {
    ...
    formData.append('workingPlaceId', bulkUploadData.workingPlaceId);
    formData.append('instituteTypeId', bulkUploadData.institutionTypeId);
    formData.append('financialYearId', bulkUploadData.financialYearId);
    formData.append('schemeId', bulkUploadData.schemeId);
    formData.append('quarterId', bulkUploadData.quarterId);
    formData.append('formType', formId);
    formData.append('addedBy', userDetails?._id);

    try {
      const response = await axios.post(URLS.BulkUploadDrugFormData, formData, {
        headers: {
      ...
      if (response.data) {
        // Count the successfully processed items
        const successCount = response.data.drugItems?.length || 0;
        ...
        if (successCount > 0) {
          toast.success(`${response.data.message || 'Excel processed successfully'} - ${successCount} items uploaded`);
          
          // Close the modal after successful upload
          setTimeout(() => {
            setBulkUploadModal(false);
            resetBulkUploadForm(); // Reset form data
      ...
      toast.error(error.response?.data?.message || 'Error during bulk upload');
    } finally {
  ...
  const handleSelectChange = (selectedOption, { name }) => {
    ...
    setFormData(prev => {
      ...
      if (name === "institutionTypeId") {
        ...
        newData.workingPlaceId = value
        fetchDistrictAndMandalData(value)
        if (formData.financialYearId) {
          fetchBudgetData(
            value,
            formData.financialYearId,
  ...
  const handleEditSelectChange = (selectedOption, { name }) => {
    ...
    setEditFormData(prev => {
      ...
      if (name === "institutionTypeId") {
        ...
        newData.workingPlaceId = value
        fetchDistrictAndMandalData(value, true)
        if (editFormData.financialYearId) {
          fetchBudgetData(
            value,
            editFormData.financialYearId,
  ...
  const addPopUp = () => {
    ...
    if (storedWorkingPlaceId) {
      fetchDistrictAndMandalData(storedWorkingPlaceId)
    }
  ...
  const editPopUp = async item => {
    try {
      ...
      if (item.workingPlaceId) {
        await fetchDistrictAndMandalData(item.workingPlaceId, true)
      }
      if (item.financialYearId) {
        await fetchSchemesAndQuarters(item.financialYearId, true)
      }
  ...
  return (
    <div className="page-content">
      <div className="container-fluid">
          ...
          <>
            ...
            <Card>
              <CardBody>
                ...
                {!isTableLoading && filteredItems.length > listPerPage && (
                  <div className="d-flex justify-content-between align-items-center mt-3">
                    <div className="text-muted">
                      Showing
                      {Math.min(
        ...
        {/* Bulk Upload Modal */}
        <Modal isOpen={bulkUploadModal} toggle={handleBulkUploadModalToggle} size="lg">
          <ModalHeader toggle={handleBulkUploadModalToggle}>
            <i className="bx bx-upload me-2"></i>
            Bulk Upload Drug Indents
          </ModalHeader>
          <Form onSubmit={handleBulkUploadSubmit}>
            <ModalBody>
              <Alert color="info">
                <i className="bx bx-info-circle me-2"></i>
                Upload Excel/CSV file with Drug Code and Quantity columns.  
              </Alert>
              ...
              <Row>
                <Col md={4}>
                  <FormGroup>
                    ...
                    <Input
                      type="select"
                      name="financialYearId"
                      value={bulkUploadData.financialYearId}
                      onChange={handleBulkUploadInputChange}
                      className={
                        uploadValidationErrors.financialYearId ? "is-invalid" : ""
                      }
                      required
                    >
                      <option value="">Select Financial Year</option>
                    ...
                    {uploadValidationErrors.financialYearId && (
                      <div className="invalid-feedback d-block">
                        {uploadValidationErrors.financialYearId}
                      </div>
                    )}
                ...
                <Col md={4}>
                  <FormGroup>
                    ...
                    <Select
                      name="schemeId"
                      value={schemeOptions.find(opt => opt.value === bulkUploadData.schemeId)}
                      onChange={handleBulkUploadSelectChange}
                      options={schemeOptions}
                      styles={selectStyles}
                      placeholder="Select Scheme"
                      isSearchable
                      required
                      isDisabled={!bulkUploadData.financialYearId}
                      className={
                        uploadValidationErrors.schemeId ? "is-invalid" : ""
                      }
                    />
                    {uploadValidationErrors.schemeId && (
                      <div className="invalid-feedback d-block">
                        {uploadValidationErrors.schemeId}
                      </div>
                    )}
                ...
                <Col md={4}>
                  <FormGroup>
                    ...
                    <Select
                      name="quarterId"
                      value={quarterOptions.find(opt => opt.value === bulkUploadData.quarterId)}
                      onChange={handleBulkUploadSelectChange}
                      options={quarterOptions}
                      styles={selectStyles}
                      ...
                      isDisabled={!bulkUploadData.financialYearId}
                      className={
                        uploadValidationErrors.quarterId ? "is-invalid" : ""
                      }
                    />
                    {uploadValidationErrors.quarterId && (
                      <div className="invalid-feedback d-block">
                        {uploadValidationErrors.quarterId}
                      </div>
                    )}
                ...
                <Col md={4}>
                  <FormGroup>
                    ...
                    <Select
                      name="institutionTypeId"
                      value={institutionTypeOptions.find(opt => opt.value === bulkUploadData.institutionTypeId)}
                      onChange={handleBulkUploadSelectChange}
                      options={institutionTypeOptions}
                      styles={selectStyles}
                      ...
                      required
                      className={
                        uploadValidationErrors.institutionTypeId ? "is-invalid" : ""
                      }
                    />
                    {uploadValidationErrors.institutionTypeId && (
                      <div className="invalid-feedback d-block">
                        {uploadValidationErrors.institutionTypeId}
                      </div>
                    )}
                ...
                <Col md={4}>
                  <FormGroup>
                    ...
                    <Select
                      name="workingPlaceId"
                      value={getBulkUploadCurrentPlace()}
                      onChange={handleBulkUploadSelectChange}
                      options={placeOfWorkingOptions}
                      styles={selectStyles}
                      ...
                      isSearchable
                      required
                      isDisabled={!bulkUploadData.institutionTypeId}
                      className={
                        uploadValidationErrors.workingPlaceId ? "is-invalid" : ""
                      }
                    />
                    {uploadValidationErrors.workingPlaceId && (
                      <div className="invalid-feedback d-block">
                        {uploadValidationErrors.workingPlaceId}
                      </div>
                    )}
                ...
                <Col md={4}>
                  <FormGroup>
                    ...
                    <Input
                      ...
                      disabled
                      type="text"
                      value={bulkUploadData.districtName || ""}
                      className="bg-light"
                      placeholder="Auto-filled"
                    />
                  </FormGroup>
                ...
                <Col md={4}>
                  <FormGroup>
                    ...
                    <Input
                      ...
                      disabled
                      type="text"
                      value={bulkUploadData.mandalName || ""}
                      className="bg-light"
                      placeholder="Auto-filled"
                    />
                  </FormGroup>
                ...
                <Col md={8}>
                  <FormGroup>
                    <Label for="file" className="fw-bold">
                    ...
                    <Input
                      ...
                      className={
                        uploadValidationErrors.file ? "is-invalid" : ""
                      }
                      required
                    />
                    {uploadedFileName && (
            ...
            <ModalFooter>
              <Button color="secondary" onClick={handleBulkUploadModalToggle} type="button">
                Cancel
              </Button>
          ...
          <ModalBody>
            ...
            {itemToDelete && (
              <div className="mt-3 p-3 bg-light rounded">
                ...
                {itemToDelete.workingPlaceName}
                <br />
                <strong>District:</strong> {itemToDelete.districtName || "N/A"}
                <br />
                <strong>Mandal:</strong> {itemToDelete.mandalName || "N/A"}
                <br />
                <strong>Scheme:</strong> {itemToDelete.schemeName || "N/A"}
                <br />
                <strong>Quarter:</strong> {itemToDelete.quarterName || "N/A"}
