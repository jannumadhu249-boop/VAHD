...
const DistrictWiseAttendanceCountWithTime = () => {
    ...
    const fetchReport = () => {
        ...
        axios
            .post(URLS.GetAllDistrictWiseAttendanceAbstractTime, payload, {
            ...
            .then(res => {
                const rows = (res.data?.data || []).map((item, idx) => ({
                    ...
                    slot_9_10: item.slot_9_10,
                    slot_10_11: item.slot_10_11,
                    slot_after_11: item.slot_after_11,
                }))
                setReportData(rows)
    ...
    const handlePdfExport = () => {
        ...
        try {
            ...
            const headers = ["Sl. No.", "District Name", "Registered", "Present", "9 AM - 10 AM", "10 AM - 11 AM", "After 11 AM"]
            ...
            const totalslot_9_10 = reportData.reduce((sum, row) => sum + (Number(row.slot_9_10) || 0), 0)
            const totalslot_10_11 = reportData.reduce((sum, row) => sum + (Number(row.slot_10_11) || 0), 0)
            const totalslot_after_11 = reportData.reduce((sum, row) => sum + (Number(row.slot_after_11) || 0), 0)
            // total.am9_10 + total.am10_11 + total.after11;
            ...
            const tableRows = reportData.map((row, index) => [
                ...
                row.slot_9_10.toString(),
                row.slot_10_11.toString(),
                row.slot_after_11.toString(),
            ])

            // Add grand total row
            tableRows.push(["", "GRAND TOTAL", totalRegistered.toString(), totalPresent.toString(), totalslot_9_10.toString(), totalslot_10_11.toString(), totalslot_after_11.toString()])
    ...
    const totalslot_10_11 = reportData.reduce((sum, row) => sum + (Number(row.slot_10_11) || 0), 0)
    const totalslot_after_11 = reportData.reduce((sum, row) => sum + (Number(row.slot_after_11) || 0), 0)
    // const grandTotal = total.am9_10 + total.am10_11 + total.after11;

    const csvData = [
        ...reportData,
        { id: "", district: "GRAND TOTAL", registered: totalRegistered, present: totalPresent, slot_9_10: totalslot_9_10, slot_10_11: totalslot_10_11, slot_after_11: totalslot_after_11 }
    ]
    ...
    const csvReport = {
        ...
        headers: [
            ...
            { label: "9 AM - 10 AM", key: "slot_9_10" },
            { label: "10 AM - 11 AM", key: "slot_10_11" },
            { label: "After 11 AM", key: "slot_after_11" },
        ],
    }
            ...
            {(Roles?.DetailAttendanceReportView === true || Roles?.accessAll === true) && (
                <div className="page-content">
                    <div className="container-fluid">
                        <Breadcrumbs title="VAHD ADMIN" breadcrumbItem="District Wise Attendance Abstract With Time Count" />
                                    ...
                                    <CardBody>
                                        <div className="d-flex align-items-center justify-content-between mb-4 mt-2">
                                            ...
                                            <div className="text-center">
                                                <h4 style={{ fontWeight: "bold", fontSize: "24px", margin: 0 }}>District Wise Attendance Abstract With Time Count</h4>
                                            </div>
                                                ...
                                                <Table bordered hover className="mb-0 text-center" style={{ borderCollapse: "collapse" }}>
                                                    <thead style={{ backgroundColor: "#c5e0b4", color: "#000" }}>
                                                        <tr style={{ cursor: "pointer" }}>
                                                            ...
                                                            {/* <th style={{ width: "15%" }} onClick={() => requestSort('present')}>Present {getSortIcon('present')}</th> */}
                                                            <th style={{ width: "20%" }} onClick={() => requestSort('present')}>9 AM - 10 AM {getSortIcon('present')}</th>
                                                            <th style={{ width: "20%" }} onClick={() => requestSort('present')}>10 AM - 11 AM {getSortIcon('present')}</th>
                                                            <th style={{ width: "20%" }} onClick={() => requestSort('present')}>After 11 AM {getSortIcon('present')}</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        {sortedData.length > 0 ? (
                                                            <>
                                                                {sortedData.map((row, index) => (
                                                                    <tr key={index}>
                                                                        ...
                                                                        <td>{row.slot_9_10}</td>
                                                                        <td>{row.slot_10_11}</td>
                                                                        <td>{row.slot_after_11}</td>
                                                                    </tr>
                                                                ))}
                                                                {/* Grand Total Row */}
                                                                <tr style={{ fontWeight: "bold", backgroundColor: "#f8f9fa" }}>
                                                                    ...
                                                                    {/* <td>{totalPresent}</td> */}
                                                                    <td>{totalslot_9_10}</td>
                                                                    <td>{totalslot_10_11}</td>
                                                                    <td>{totalslot_after_11}</td>
                                                                </tr>
                                                            </>
                                                        ) : (
                                                            <tr>
...
export default DistrictWiseAttendanceCountWithTime
