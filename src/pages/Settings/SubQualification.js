...
const SubQualification = () => {
  ...
  const Get = () => {
    ...
    axios
      ...
      .catch(error => {
        console.error("Error fetching SubQualification:", error)
      })
  }
  ...
  const SearchData = e => {
    ...
    const token = TokenData
    axios
      .get(URLS.GetSubQualificationSearch + value, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(res => {
        if (res.status === 200) {
  ...
  const AddData = () => {
    ...
    axios
      .post(URLS.AddSubQualification, dataArray, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(res => {
        if (res.status === 200) {
  ...
  const UpdateData = () => {
    ...
    const dataArray = {
      name: form1.name,
      qualificationId: form1.qualificationId,
    }

    axios
      .put(URLS.EditSubQualification + form1._id, dataArray, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(res => {
        if (res.status === 200) {
  ...
  const Delete = data => {
    ...
    axios
      .delete(URLS.DeleteSubQualification + remid, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(res => {
        if (res.status === 200) {
    ...
    <React.Fragment>
      <div className="page-content">
        <div className="container-fluid">
          <Breadcrumbs title="VAHD ADMIN" breadcrumbItem="Specialization" />
          <Row>
            {/* Add Form */}
            <Col md={12}>
              {show && (
                        ...
                        {lists.map((data, key) => (
                          <tr key={key} className="text-center">
                            ...
                            <td>
                              ...
                              {Roles?.SpecializationDelete === true ||
                              Roles?.accessAll === true ? (
                                <>
                                  <Button
                                    size="md"
