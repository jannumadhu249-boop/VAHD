...
const VaccinationType = () => {
  ...
  const Get = () => {
    ...
    axios
      ...
      .catch(error => {
        console.error("Error fetching VaccinationType:", error)
      })
  }
  ...
  const SearchData = e => {
    ...
    const token = TokenData
    axios
      .get(URLS.GetVaccinationTypeSearch + value, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(res => {
        if (res.status === 200) {
  ...
  const AddData = () => {
    ...
    axios
      .post(URLS.AddVaccinationType, dataArray, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(res => {
        if (res.status === 200) {
  ...
  const UpdateData = () => {
    ...
    const dataArray = {
      name: form1.name,
      animalTypeId: form1.animalTypeId,
    }

    axios
      .put(URLS.EditVaccinationType + form1._id, dataArray, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(res => {
        if (res.status === 200) {
  ...
  const Delete = data => {
    ...
    axios
      .delete(URLS.DeleteVaccinationType + remid, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(res => {
        if (res.status === 200) {
    ...
    <React.Fragment>
      <div className="page-content">
        <div className="container-fluid">
          <Breadcrumbs title="VAHD ADMIN" breadcrumbItem="Vaccination Type" />
          <Row>
            {/* Add Form */}
            <Col md={12}>
              {show && (
                      ...
                      <tbody>
                        {lists.map((data, key) => (
                          <tr key={key} className="text-center">
                            ...
                            <td>{data.name}</td>
                            <td>{data.animalTypeName}</td>
                            <td>
                              {Roles?.VaccinationTypeEdit === true ||
                              Roles?.accessAll === true ? (
                                <>
                                  <Button
                              ...
                              {Roles?.VaccinationTypeDelete === true ||
                              Roles?.accessAll === true ? (
                                <>
                                  <Button
                                    size="md"
