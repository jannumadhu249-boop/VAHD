...
const LeaveReason = () => {
    ...
    const Get = () => {
        axios
            ...
            .then(res => {
                if (res.data.success) {
                    setData(res.data.leavereasons || [])
                    setFilteredData(res.data.leavereasons || [])
                }
            })
            .catch(error => {
    ...
    const AddData = () => {
        ...
        axios
            .post(URLS.AddLeaveReason, dataArray, {
                headers: { Authorization: `Bearer ${token}` },
            })
            .then(
                res => {
    ...
    const UpdateData = () => {
        ...
        axios
            .put(URLS.EditLeaveReason + editId, dataArray, {
                headers: { Authorization: `Bearer ${token}` },
            })
            .then(
                res => {
    ...
    const handleDelete = id => {
        ...
        if (confirmBox === true) {
            axios
                .delete(URLS.DeleteLeaveReason + id, {
                    headers: { Authorization: `Bearer ${token}` },
                })
                .then(
                    res => {
        ...
        <React.Fragment>
            <div className="page-content">
                <div className="container-fluid">
                    <Breadcrumbs title="VAHD ADMIN" breadcrumbItem="Leave Reasons" />

                    <Row>
                        <Col md={4}>
                            <Card className="p-4">
                                <h5 className="mb-3">
                                    {isEdit ? "Edit Leave Reason" : "Add Leave Reason"}
                                </h5>
                        ...
                        <Col md={8}>
                            <Card>
                                <CardBody>
                                    <Row className="mb-3">
                                        <Col md={6}>
                                            <h5 className="mb-3">Leave Reasons List</h5>
                                        </Col>
                                        <Col md={6}>
                                            <Input
                                                type="text"
