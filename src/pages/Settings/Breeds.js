...
const Breeds = () => {
  ...
  const SearchData = e => {
    ...
    const token = TokenData
    axios
      .get(URLS.GetBreedsSearch + value, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(res => {
        if (res.status === 200) {
  ...
  const UpdateData = () => {
    ...
    axios
      .put(URLS.EditBreeds + form1._id, dataArray, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(res => {
        if (res.status === 200) {
  ...
  const Delete = data => {
    ...
    axios
      .delete(URLS.DeleteBreeds + remid, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(res => {
        if (res.status === 200) {
    ...
    <React.Fragment>
      <div className="page-content">
        <div className="container-fluid">
          <Breadcrumbs title="VAHD ADMIN" breadcrumbItem="Breeds" />
          <Row>
            {/* Add Form */}
            <Col md={12}>
              {show && (
            ...
            <Col md={12}>
              <Card>
                <CardBody>
                  <Row>
                    <Col>
                      {Roles?.BreedsAdd === true ||
                      Roles?.accessAll === true ? (
                        <>
                          <Button color="primary text-white" onClick={AddPopUp}>
                            Create Breeds <i className="bx bx-plus-circle"></i>
                          </Button>
                        </>
                      ) : (
                        ""
