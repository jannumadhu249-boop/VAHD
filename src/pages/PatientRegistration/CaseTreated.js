...
//     ageMonths: "",
//     noOfUnits: "",
//     diagnostics: "Case Treated",
//     remarks: "",
//     ownerName: "",
//     ownerMobile: "",
//     workingPlaceId: "",
//     formType: "casetreated",
//     search: "",
//     caste: "",
//     aadharNo: "",
//     Village: "",
...
//       toast.error("Failed to load Case Treated data")
//     }
//   }, [token])
...
//         toast.error(error.response.data.message)
//       } else {
//         toast.error("Failed to add case treated")
//       }
//     }
//   }
...
//         toast.error(error.response.data.message)
//       } else {
//         toast.error("Failed to update case treated")
//       }
//     }
//   }
...
//         toast.error(error.response.data.message)
//       } else {
//         toast.error("Failed to delete case treated")
//       }
//     }
//   }
...
//           <Breadcrumbs title="VAHD ADMIN" breadcrumbItem="Case Treated" />
//           <Row>
//             <Col md={12}>
//               {show && (
//                 <Card className="mb-4">
...
const CaseTreated = () => {
  ...
  const handleAnimalDetailChange = (index, field, value) => {
    setForm(prev => {
      ...
      if (index === prev.animalDetails.length - 1) {
        ...
        if (currentDetail.animalTypeId && currentDetail.noOfUnits) {
  ...
  const handleUnitsChange = (e, index) => {
    ...
    handleAnimalDetailChange(index, 'noOfUnits', value)
  }
  ...
  const addData = async () => {
    ...
    const invalidAnimalDetails = form.animalDetails.filter(detail =>
      !detail.animalTypeId || !detail.noOfUnits
    )

    if (invalidAnimalDetails.length > 0) {
      toast.error("Please fill all animal type and number of units fields")
      return
    }
    ...
    if (validAnimalDetails.length === 0) {
      toast.error("Please add at least one animal detail")
      return
    }
  ...
  const handleUnitsChange1 = (e, index) => {
    const { value } = e.target
    handleAnimalDetailChange1(index, 'noOfUnits', value)
  }
  ...
  const updatePopUp = async data => {
    try {
      ...
      let animalDetails = []
      if (data.animalDetails && Array.isArray(data.animalDetails)) {
        animalDetails = data.animalDetails.map(detail => ({
      ...
      // Only add one if there are no animal details at all.
  ...
  const updateData = async () => {
    ...
    if (validAnimalDetails.length === 0) {
      toast.error("Please fill at least one animal type and number of units")
      return
    }
                              ...
                              <Col md={5}>
                                <FormGroup>
                                  ...
                                  <Input
                                    ...
                                    inputMode="decimal"
                                    onWheel={(e) => e.target.blur()}
                                    name={`noOfUnits-${index}`}
                                    value={detail.noOfUnits}
                              ...
                              <Col md={5}>
                                <FormGroup>
                                  ...
                                  <Input
                                    ...
                                    name={`noOfUnits-${index}`}
                                    value={detail.noOfUnits}
                                    onChange={(e) => handleUnitsChange1(e, index)}
                                    min="1"
                                    placeholder="No of Units"
                                    required
                                  />
                        ...
                        <tbody>
                          {lists.length > 0 ? (
                            lists.map((data, index) => {
                              ...
                              if (data.animalDetails && Array.isArray(data.animalDetails)) {
                                animalDetailsList = data.animalDetails
                                totalUnits = data.animalDetails.reduce((sum, detail) =>
                                  sum + (parseInt(detail.noOfUnits) || 0), 0
                                )
                              } else {
                                animalDetailsList = [{
                                ...
                                totalUnits = parseInt(data.noOfUnits) || 0
                              }
                                    ...
                                    <div>
                                      {animalDetailsList.map((detail, idx) => (
                                        <div key={idx} className="mb-1">
                                          ...
                                          <br />
                                          <strong>Units {idx + 1}:</strong> {detail.noOfUnits || "0"}
